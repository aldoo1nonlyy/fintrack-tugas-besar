...
class _FinanceScreenState extends State<FinanceScreen> {
  ...
  void _showFinanceForm(BuildContext context) {
    showFinanceFormSheet(context);
  }
}

void showFinanceFormSheet(BuildContext context, {FinancialEntry? entry}) {
  showModalBottomSheet<void>(
    context: context,
    isScrollControlled: true,
    useSafeArea: true,
    builder: (_) => FinanceFormSheet(entry: entry),
  );
}
...
class FinanceFormSheet extends StatefulWidget {
  ...
  const FinanceFormSheet({super.key, this.entry});
...
class _FinanceFormSheetState extends State<FinanceFormSheet> {
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 18),
              Text(
                widget.entry == null ? 'Tambah catatan keuangan' : 'Edit catatan keuangan',
                style: Theme.of(context).textTheme.titleLarge,
              ),
  ...
  void _save() {
    ...
    if (widget.entry == null) {
      provider.addFinancialEntry(
        FinancialEntry(
          ...
          date: DateTime.now(),
          type: _type,
          status: (_type == FinancialEntryType.payable || _type == FinancialEntryType.receivable) ? TransactionStatus.unpaid : null,
        ),
      );
    } else {
      provider.updateFinancialEntry(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(widget.entry == null ? 'Catatan keuangan berhasil ditambahkan' : 'Catatan keuangan berhasil diperbarui')),
    );
  }
